...
function RecipeIngridients({ recipeData }) {
  ...
  return (
    <div className="ingredients__container">
      ...
      <h3 className="ingredients__length">{` (${ingredients.length})`}</h3>
      {ingredients.map((element, index) => {
        return (
          <div className="ingredients__ingredients" key={index}>
            <span className="ingredients__food">{element.food}</span>
            <span className="test">
              <span className="ingredients__quantity">{`${element.quantity.toFixed(
                1
              )} `}</span>
              {element.measure && (
                <span className="ingredients__measure">{`${
                  element.measure == "<unit>" ? "unit" : element.measure
                } `}</span>
              )}
            </span>
          </div>
